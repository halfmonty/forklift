...
## Contents
- [PDNA tool implementation plan](docs/PDNA_TOOL_IMPLEMENTATION_PLAN.md) -- plan for the public native Playdate audio authoring and export tool.
- [PDNA Zig adapter implementation plan](docs/PDNA_ADAPTER_IMPLEMENTATION_PLAN.md) -- integration boundary for PDNA-exported Zig source.
- [PDNA expressive native audio plan](docs/PDNA_EXPRESSIVE_AUDIO_PLAN.md) -- staged v2 contract for musical expression and percussion kits.
...
The browser port is a static WebAssembly build for browsers that support
WebAssembly, Canvas 2D, `localStorage`, and Web Audio. The native Playdate
build remains the reference target.

Build the production directory with:

`zig build web`

This writes `index.html`, `main.js`, `style.css`, and
`forklift-certified.wasm` to `zig-out/web/`; deploy that directory to any
static HTTP host. For local development, run `zig build web-dev` and open
`http://localhost:8001`. If that port is occupied, choose another one, for
example:

`zig build web-dev -Dweb-port=8010`

After the Start Console gesture, the browser build synthesizes both gameplay
effects and the PDNA music sequence through Web Audio.

Browser controls are the arrow keys for driving and forks, `Z` or Space for
action, `X` for camera, `Q`/`E` or the crank slider for crank input, and `M`
for the service panel. The canvas must have focus for game controls; clicking
Start Console focuses it automatically. Campaign-save API calls are stored in
the browser's `localStorage`, so normal campaign progress remains available
after reload in the same browser.

Run `zig build test` for host tests and `zig build web-test` for the built-WASM
integration check. The latter validates static artifacts, game startup,
framebuffer dimensions and rendering, deterministic title/briefing/gameplay
framebuffer baselines, an action input frame, and the PDNA sequence bridge.
